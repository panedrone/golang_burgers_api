...
function fetchSearchResults() {
    let b = _burger_name !== null && _burger_name.length > 1;
    let i = _ingredient_name !== null && _ingredient_name.length > 1;
    if (!b && !i) {
        return
    }
        ...
        <table className="w100">
            ...
            <tr>
                <td className="v-middle">
                    <StringField onChange={(v) => _burger_name = v} saveUpdater={(u) => _burger_set = u} placeholder={"Type a Search Key of Burger Name"}/>
                </td>
                <td className="v-middle w1">
                    <input type={"button"} value={"x"} onClick={() => _burger_set("")}/>
                </td>
                <td className="v-middle">
                    <StringField onChange={(v) => _ingredient_name = v} saveUpdater={(u) => _ingredient_set = u}  placeholder={"Type an Search Key of Ingredient"}/>
                </td>
                <td className="v-middle w1">
                    <input type={"button"} value={"x"} onClick={() => _ingredient_set("")}/>
                </td>
                <td className="v-middle w1">
                    <input type={"button"} value={"Go"} onClick={() => fetchSearchResults()}/>
                </td>
            </tr>
            </tbody>
        </table>
